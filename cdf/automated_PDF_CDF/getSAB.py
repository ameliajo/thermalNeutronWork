...
def getSAB(alphas,betas,T,rho,NJOY_LEAPR,fullRedo,width,oscE=None,oscW=None):
    ...
    if NJOY_LEAPR:
        # Run NJOY LEAPR
        name = 'Delta_'+str(T) if width == None else 'Contin_'+str(T)+'_'+str(width)
        if fullRedo:
            ...
            generateNjoyInput(name,alphas,betas,rho,width == None,T)
            runNJOY(name)
        ...
        # Run my own LEAPR
        name = 'sab_Delta_'+str(T)+'.txt' if width == None else 'sab_Contin_'+str(T)+'_'+str(width)+'.txt'
        if fullRedo:
            if width == None:
                assert(oscE != None and oscW != None)
                writeRho('inputVals.txt',continRho,alphas,betas,oscE,oscW,T)
                ...
                assert(oscE != None and oscW != None)
                writeRho('inputVals.txt',getPhononDist(width,continRho),alphas,betas,[],[],T)
                subprocess.run(['g++','-std=c++14','./MY_LEAPR/deltaFuncLEAPR.cpp'])
                subprocess.run(['./a.out'])
                subprocess.run(['mv','inputVals.txt','./MY_LEAPR/alphaBetaRhoInputs/'+name])
                subprocess.run(['mv','outputSAB.txt','./MY_LEAPR/sabResults/'+name])
...
if __name__=="__main__":
    ...
    fullRedo = True
    width = None 
    MY_DELTA_SAB   = getSAB(alphas,betas,296.0,continRho,NJOY_LEAPR=False,\
                            fullRedo=fullRedo,width=width,oscE=oscE,oscW=oscW)
    NJOY_DELTA_SAB = getSAB(alphas,betas,296.0,continRho,NJOY_LEAPR=True,\
                            fullRedo=fullRedo,width=width,oscE=oscE,oscW=oscW)
