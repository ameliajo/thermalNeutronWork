...
def getSABval(sab,a,b,n_beta):
    return sab[a*n_beta+b]#*np.exp(betas[b]/2)
...
def calcIntegralAcrossAlpha(A,E,T,beta,b):
    ...
    for a in range(len(alphas)-1):
        ...
        #sabL = getSABval(sab,a,b,n_beta)
        #sabR = getSABval(sab,a+1,b,n_beta)
...
def calcBetaPDF(A,E,T):
    ...
    for b in range(len(betas)-1):
        denominator += ( calcIntegralAcrossAlpha(A,E,T,betas[b],b) +    \
                         calcIntegralAcrossAlpha(A,E,T,betas[b+1],b+1) ) *\
                       0.5 * ( betas[b+1]-betas[b] )

    eq14 = [0.0]*len(betas)
    for b in range(len(betas)):
        eq14[b] = calcIntegralAcrossAlpha(A,E,T,betas[b],b) / denominator
    return betas, eq14
